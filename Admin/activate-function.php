...
?>
   ...
   if(isset($_GET['record'])) {
	if (isset($_GET['roll_no']) && isset($_GET['record'])) {
		$roll_no=$_GET['roll_no'];
		$query1="update login set account = 'Activate' where user_id='$roll_no'"; 
        //echo  $query1; die("==============++");  
	...
	if (isset($_GET['roll_no'])) {  
		$roll_no=$_GET['roll_no'];
		$query1="update login set account = '' where user_id='$roll_no'"; 
        //echo  $query1; die("==============++");  
		$run1=mysqli_query($con,$query1);  
		if ($run1) {
			header('Location: student.php');
